...
SELECT
    ...
    COUNT(DISTINCT d.primaryid) AS report_count
FROM faers_drug d
JOIN faers_outc o ON d.primaryid = o.primaryid
...
SELECT
    ...
    d.prod_ai_clean,
    d.quarter,
    COUNT(DISTINCT d.primaryid)     AS death_reports,
    COUNT(DISTINCT d.primaryid)::FLOAT /
        NULLIF((SELECT COUNT(DISTINCT primaryid) FROM faers_drug
                WHERE role_cod = 'PS'), 0) * 100  AS pct_of_all_ps_reports
FROM faers_drug d
JOIN faers_outc o ON d.primaryid = o.primaryid
WHERE o.outc_cod = 'DE'
...
SELECT
    ...
    dem.quarter,
    COUNT(DISTINCT dem.primaryid)                                          AS report_count,
    COUNT(DISTINCT CASE WHEN o.outc_cod = 'DE' THEN dem.primaryid END)    AS death_count,
    COUNT(DISTINCT CASE WHEN o.outc_cod = 'HO' THEN dem.primaryid END)    AS hospitalization_count,
    COUNT(DISTINCT CASE WHEN o.outc_cod = 'LT' THEN dem.primaryid END)    AS life_threatening_count
FROM faers_demo dem
LEFT JOIN faers_outc o ON dem.primaryid = o.primaryid
...
totals AS (
    SELECT COUNT(DISTINCT primaryid) AS N FROM faers_demo
),
...
drug_reac_pairs AS (
    SELECT
        ...
        r.pt_clean AS reaction_term,
        d.quarter,
        COUNT(DISTINCT d.primaryid) AS a    -- reports with BOTH drug AND reaction
    FROM faers_drug d
    JOIN faers_reac r ON d.primaryid = r.primaryid
    WHERE d.role_cod = 'PS'
...
SELECT
    dem.quarter,
    COUNT(DISTINCT dem.primaryid)                                       AS total_cases,
    COUNT(DISTINCT CASE WHEN o.outc_cod = 'DE' THEN dem.primaryid END)  AS deaths,
    COUNT(DISTINCT CASE WHEN o.outc_cod = 'HO' THEN dem.primaryid END)  AS hospitalizations,
    COUNT(DISTINCT CASE WHEN o.outc_cod = 'LT' THEN dem.primaryid END)  AS life_threatening,
    COUNT(DISTINCT CASE WHEN dem.sex = 'Female' THEN dem.primaryid END) AS female_cases,
    COUNT(DISTINCT CASE WHEN dem.sex = 'Male' THEN dem.primaryid END)   AS male_cases,
    AVG(dem.age_years)                                                   AS avg_age,
...
SELECT
    ...
    -- Breakdown by role
    COUNT(DISTINCT CASE WHEN role_cod = 'PS' THEN primaryid END) AS ps_count,
    COUNT(DISTINCT CASE WHEN role_cod = 'SS' THEN primaryid END) AS ss_count,
    COUNT(DISTINCT CASE WHEN role_cod = 'C'  THEN primaryid END) AS concomitant_count
FROM faers_drug
WHERE drugname_clean IS NOT NULL AND drugname_clean != ''
...
SELECT
    ...
    COUNT(DISTINCT primaryid) AS report_count
FROM faers_reac
WHERE pt_clean IS NOT NULL AND pt_clean != ''
GROUP BY pt_clean, quarter;
